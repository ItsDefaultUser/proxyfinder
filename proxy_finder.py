...
import time
# Fake user agents list for user agent spoofing; this is necessary because the provider website does not give access to robots or web scrapers.  To bypass their robots exclusion protocol, a fake user agent may be used to identify your browser as a legitimate one.
user_agents = [
...
## This is a very short class that overrides urllib's FancyURLopener, so the version of the urlopener can be modified to contain a random choice from user_agents
